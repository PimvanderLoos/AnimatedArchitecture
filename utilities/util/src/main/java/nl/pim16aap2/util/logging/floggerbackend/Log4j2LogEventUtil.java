 ...
 * named log4j implementation. If the behaviour of this class starts to deviate from that of the log4j backend in any
 * significant way, this difference should be called out clearly in the documentation.
 * <p>
 * This class was modified to add more log levels than the default ones. The levels used by FLogger do not all exist in
 * Log4j2, so we added them ourselves. This allows us to more finely control logging. Previously,
 * {@link FluentLogger#atFinest()} and {@link FluentLogger#atFiner()} would both log at the same level and so would
 * {@link FluentLogger#atFine()} and {@link FluentLogger#atConfig()}. Now, all four methods log at different levels.
 * <p>
 * Specifically, the following levels were added:
 * <ul>
...
public final class Log4j2LogEventUtil
{
    ...
    /**
     * Formats the log message in response to an exception during a previous logging attempt. A synthetic error message
     * is generated from the original log data and the given exception is set as the cause. The level of this record is
     * the maximum of WARNING or the original level.
     */
     ...
     * logs which are written from current thread. This context data will be added to the log4j2 event.
     */
