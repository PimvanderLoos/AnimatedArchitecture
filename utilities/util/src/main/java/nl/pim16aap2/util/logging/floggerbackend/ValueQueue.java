...
/**
 * A simple FIFO queue linked-list implementation designed to store multiple metadata values in a StringMap. There are
 * two aspects worth pointing out:
 *
 ...
 * Because Log4j2 uses a {@code String} keyed map, we would risk clashing of values if we just used the label to store
 * each value directly. This class lets us store a list of values for a single label while being memory efficient in the
 * common case where each label really does only have one value.
 */
...
final class ValueQueue implements Iterable<Object>
{
    ...
    /**
     * Helper method for creating and initializing a value queue with a non-nullable value. If value is an instance of
     * Tags, each tag will be added to the value queue.
     */
    ...
    /**
     * Emits a metadata label/value pair to a given {@code KeyValueHandler}, handling {@code Tags} values specially.
     *
     * <p>Tags are key-value mappings which cannot be modified or replaced. If you add the tag mapping
     * {@code "foo" -> true} and later add {@code "foo" -> false}, you get "foo" mapped to both true and false. This is
     * very deliberate since the key space for tags is global and the risk of two bits of code accidentally using the
     * same tag name is real (e.g. you add "id=xyz" to a scope, but you see "id=abcd" because someone else added
     ...
     * Log4j2's ContextMap.
     */
    static void emit(@NullableDecl String label, Object value, MetadataKey.KeyValueHandler kvh)
    {
