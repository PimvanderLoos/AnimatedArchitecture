...
/**
 * v1_19_R1 implementation of {@link IAnimatedBlockFactory}.
 *
 * @author Pim
 * @see IAnimatedBlockFactory
 */
@Singleton
public final class AnimatedBlockFactory_V1_19_R1 implements IAnimatedBlockFactory
{
    private final AnimatedBlockHookManager animatedBlockHookManager;

    AnimatedBlockFactory_V1_19_R1(AnimatedBlockHookManager animatedBlockHookManager)
    {
        ...
        throws Exception
    {
        ...
        if (!BlockAnalyzer_V1_19_R1.isAllowedBlockStatic(material))
            return Optional.empty();

        final double offset = bottom ? 0.010_001 : 0;
        final IPLocation spawnLoc = loc.add(0, offset - 0.020, 0);

        final var animatedBlock = new CustomEntityFallingBlock_V1_19_R1(
