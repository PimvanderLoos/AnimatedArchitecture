...
public class PlotSquared5ProtectionCompat implements IProtectionCompat
{
    ...
    private com.plotsquared.core.location.Location convertLocation(Location loc)
    {
        // The BukkitUtil class has the same path in both PS3 and PS5, so it will look like
        // the returned location is actually of the type com.intellectualcrafters.plot.object.Location
        // Instead of the required type: com.plotsquared.core.location.Location
        // Because yes, those do NOT share a package between versions -_-
        Object tmpLocation = com.plotsquared.bukkit.util.BukkitUtil.getLocation(loc);
        //noinspection ConstantConditions
        return (com.plotsquared.core.location.Location) tmpLocation;
    }
    ...
    {
        com.plotsquared.core.location.Location psLocation = convertLocation(loc);
    ...
    {
        com.plotsquared.core.location.Location psLocation = convertLocation(loc1);
        ...
        for (int xPos = x1; xPos <= x2; ++xPos)
            for (int zPos = z1; zPos <= z2; ++zPos)
            {
                ...
                psLocation = convertLocation(loc);
                com.plotsquared.core.plot.PlotArea area = psLocation.getPlotArea();
                if (area == null)
                    continue;
