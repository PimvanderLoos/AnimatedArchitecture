...
public class CommandHandler implements CommandExecutor
{
    ...
    public void listDoors(Player player, String doorName)
    {
        ...
        for (Door door : doors)
            Util.messagePlayer(player, door.getBasicInfo());
        if (doors.size() == 0)
    ...
    public void listDoorInfo(Player player, String name)
    {
        ...
        // If a door
        if (doorUID != -1)
            Util.messagePlayer(player, plugin.getCommander().getDoor(player.getUniqueId(), doorUID).getFullInfo());
        else
        {
            ArrayList<Door> doors = plugin.getCommander().getDoors(player.getUniqueId().toString(), name);
            for (Door door : doors)
                Util.messagePlayer(player, door.getFullInfo());
            if (doors.size() == 0)
                Util.messagePlayer(player, plugin.getMessages().getString("GENERAL.NoDoorsFound"));
        }
    }
    ...
    public void listDoorInfoFromConsole(String str)
    {
        ...
        if (doorUID == -1)
        {
            ...
            for (Door door : doors)
                plugin.getMyLogger().myLogger(Level.INFO, plugin.getCommander().playerNameFromUUID(door.getPlayerUUID()) + ": " + door.getFullInfo());
            if (doors.size() == 0)
        ...
        plugin.getMyLogger().myLogger(Level.INFO, Util.nameFromUUID(door.getPlayerUUID()) + ": " + door.getFullInfo());
    }
    ...
    private void listDoorsFromConsole(String playerUUID, String name)
    {
        ...
        for (Door door : doors)
        {
            ...
            plugin.getMyLogger().myLogger(Level.INFO,
                                          (playerName == null ? "" : playerName + ": ") + door.getBasicInfo());
        }
        if (doors.size() == 0)
            plugin.getMyLogger().myLogger(Level.INFO, plugin.getMessages().getString("GENERAL.NoDoorsFound"));
    }
