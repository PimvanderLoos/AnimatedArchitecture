...
public final class MovableOpeningHelper
{
        ...
        IMessageable messageReceiver)
    {
        log.at(Level.FINE).log("Aborted toggle for movable %d because of %s. Toggle Reason: %s, Responsible: %s",
                               movable.getMovableUID(), result.name(), cause.name(), responsible.asString());
        ...
        if (!result.equals(MovableToggleResult.BUSY))
            doorActivityManager.setMovableAvailable(movable.getMovableUID());
        ...
        if (!result.equals(MovableToggleResult.NO_PERMISSION))
        {
            ...
            else
            {
                ...
                log.at(level).log("Failed to toggle movable: %d, reason: %s", movable.getMovableUID(), result.name());
            }
        }
        return result;
    }
        ...
        return protectionCompatManager.canBreakBlocksBetweenLocs(responsible, cuboid.getMin(), cuboid.getMax(),
            ...
            protectionCompat ->
            {
                log.at(Level.WARNING).log("Player '%s' is not allowed to open movable '%s' (%d) here! Reason: %s",
                                          responsible, movable.getName(), movable.getMovableUID(), protectionCompat);
                return false;
            }).orElse(true);
    }
    ...
    {
        if (!doorActivityManager.attemptRegisterAsBusy(movable.getMovableUID()))
