...
/**
 * Represents a manager for handling movable specifications requests.
 *
 * @author Pim
 */
@Singleton
public final class MovableSpecificationManager extends Restartable
{
    private final Map<IPPlayer, DelayedInputRequest<String>> requests = new ConcurrentHashMap<>();

    @Inject
    public MovableSpecificationManager(RestartableHolder holder)
    {
        super(holder);
    }

    /**
     * Checks if there is an open movable specification request for a player.
     *
     * @param player
     *     The player to check.
     * @return True if there is an open movable specification request for the player.
     */
