...
public enum SQLStatement
{
    ...
    GET_DOORS_WITH_NAME(
        "SELECT *\n" +
            "FROM DoorBase AS D INNER JOIN DoorOwnerPlayer as U ON D.id = U.doorUID INNER JOIN Player AS P ON U.playerID = P.id\n" +
    ...
    GET_NAMED_DOORS_OWNED_BY_PLAYER(
        "SELECT D.*, P.playerName, P.playerUUID, U.permission \n" +
            "FROM DoorOwnerPlayer AS U INNER JOIN Player AS P ON U.playerID = P.id INNER JOIN DoorBase AS D ON U.doorUID = D.id \n" +
    ...
    GET_POWER_BLOCK_DATA(
        "SELECT * FROM DoorBase WHERE chunkHash=?;"),

    GET_DOORS_IN_CHUNK(
        "SELECT * FROM DoorBase WHERE chunkHash=?;"),

    UPDATE_BLOCKS_TO_MOVE(
        "UPDATE DoorBase SET blocksToMove=? WHERE id=?;"),
    ...
    UPDATE_DOOR_AUTO_CLOSE(
        "UPDATE DoorBase SET autoClose=? WHERE id=?;"),
...
//    INSERT_DOOR(
//        "INSERT INTO DoorBase(name,world,xMin,yMin,zMin,xMax,yMax,zMax, \n" +
